...
export default function RedoxAffiliate() {
  const [, slugParams] = useRoute("/redox/:slug");
  const [location] = useLocation();
  // Support both /redox (no slug = owner fallback) and /redox/:slug
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    submitEnquiry.mutate({
      affiliateSlug: affiliate?.slug ?? slug,
  ...
  // After guard, affiliate is guaranteed to be defined
  const aff = affiliate!;

  const socials = [
    { platform: "facebook", url: aff.facebook },
    { platform: "instagram", url: aff.instagram },
    { platform: "linkedin", url: aff.linkedin },
    { platform: "youtube", url: aff.youtube },
  ].filter(s => s.url);
    ...
    <div className="min-h-screen bg-[#0a0f1e] text-white font-sans">
      ...
      <section id="products" className="py-20 bg-[#0a0f1e]">
        <div className="max-w-6xl mx-auto px-4">
          <div className="text-center mb-14">
            <h2 className="text-3xl font-bold mb-4">Two ASEA Flagship Products</h2>
            <p className="text-gray-400 max-w-2xl mx-auto">
              Two clinically studied products from ASEA's broader range. One for inside your body, one for your skin.
            </p>
          </div>
      ...
      <section id="contact" className="py-20 bg-[#060c1a]">
        <div className="max-w-2xl mx-auto px-4">
          <div className="text-center mb-10">
            <h2 className="text-3xl font-bold mb-3">Talk to {aff.name.split(" ")[0]}</h2>
            <p className="text-gray-400">
              Have questions about ASEA products? {aff.name.split(" ")[0]} is here to help.
            </p>
          </div>
          ...
          {formSent ? (
            <div className="bg-green-500/10 border border-green-500/30 rounded-2xl p-10 text-center">
              ...
              <p className="text-gray-400">
                {aff.name.split(" ")[0]} will be in touch with you shortly.
              </p>
            </div>
          ) : (
