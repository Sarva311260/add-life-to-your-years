...
import {
  ...
  Gift, HeartHandshake, ExternalLink, ShoppingBag,
} from "lucide-react";
...
export default function HomeAffiliate() {
  ...
  const { data: affiliate } = trpc.pemfAffiliate.getBySlug.useQuery(
    { slug },
    { enabled: !!slug, retry: false }
  );

  const affiliateName = affiliate?.name || null;

  // Fetch recommended products with this affiliate's links
  const { data: recommendedProducts = [] } = trpc.recommendedProducts.list.useQuery(
    { affiliateSlug: slug || undefined },
    { enabled: !!slug, retry: false }
  );

  // Set affiliate cookie with no expiry when visitor arrives via ref link
  useEffect(() => {
    if (slug) {
      // Set a cookie that never expires (far future date)
    ...
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? "bg-white/95 backdrop-blur-md shadow-sm" : "bg-transparent"}`}>
        <div className="container flex items-center justify-between h-16">
          <Link href={`/ref/${slug}`} className="flex items-center gap-2">
            <img src="/manus-storage/logo-favicon_d30c3a72.png" alt="Add Life to Your Years" className="w-10 h-10 object-contain" />
            <span className={`font-serif text-lg font-semibold ${scrolled ? "text-foreground" : "text-white"}`}>Add Life to Your Years</span>
          </Link>

          {/* Affiliate badge */}
          {affiliateName && (
            <div className={`hidden sm:flex items-center gap-2 px-3 py-1 rounded-full text-xs font-medium ${scrolled ? "bg-green-100 text-green-700" : "bg-white/20 text-white"}`}>
      ...
      <section className="relative pt-32 pb-20 md:pt-40 md:pb-28 overflow-hidden">
        ...
        <div className="container relative">
          <div className="max-w-3xl mx-auto text-center">
            ...
            >
              {affiliateName && (
                <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-emerald-500/30 text-white text-sm font-medium mb-4">
                  <User className="w-4 h-4" />
                  Referred by {affiliateName}
                </div>
              )}
          ...
          {recommendedProducts.length === 0 ? (
            ...
            <div className="grid md:grid-cols-3 gap-6">
              {(recommendedProducts as any[]).map((product: any, i: number) => {
                const buyUrl = product.resolvedUrl || product.defaultAffiliateUrl || product.productUrl || "#";
            ...
            <div>
              ...
              {affiliateName && (
                <p className="text-xs mt-3 text-white/40">
                  You arrived via a referral link from {affiliateName}.
                </p>
              )}
            </div>
            <div>
